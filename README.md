...
| Tab | What it does |
|---|---|
| **Auditory session** | Form for every session setting, with a parameter panel for the selected experiment mode and a live view of the stimuli. Load/save the config as YAML, draw the ROIs on the live camera, enter the mouse ID and start the session. The session's console output appears in the panel; the live tracking windows are the same OpenCV windows as before. |
...
**Designing the stimuli.** Choosing an experiment mode shows the parameters for that mode and hides the rest, so nothing on screen is a setting that would be ignored. Settings that depend on a switch behave the same way: the Arduino port appears only when TTL output is on. Each section carries a "What does this do?" toggle explaining what it controls and why it matters. The block schedule is recomputed as you type and shown as plain words, for example `15 silent + 15 active + 2 silent + ... = 83 min total`.

**Seeing the stimuli.** The panel beside the form shows what will play, generated by the real trial factory rather than an approximation:

| View | Shows |
|---|---|
| One arm in detail | Waveform with its amplitude envelope, spectrum, and spectrogram. A Play button auditions it through the configured device. |
| Compare all arms | Every arm's waveform and spectrum together, so you can see how the stimulus set differs across the maze. |

Colours come from the same palette as the session figures (`STIM_COLORS` in `amazeing/auditory/analysis.py`), so a category looks the same in the application as it does in your results: blues for the enriched-environment tiers, reds for standard, green for vocalisations, grey for silence. Plots follow the window's light or dark theme.
