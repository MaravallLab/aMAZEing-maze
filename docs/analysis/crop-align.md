# Cropping and aligning the videos

Stage 1 of the [pose estimation pipeline](pose.md). Warps every video so the maze lands in the same place, which is what lets later stages identify a rig from a video's dimensions and filter detections against the maze outline.

Run with `python analysis/crop_and_align_maze.py`.
